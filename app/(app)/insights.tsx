    ...
    <SafeAreaView className="flex-1 bg-bg">
      <ScrollView contentContainerStyle={{ paddingHorizontal: 24, paddingTop: 16, paddingBottom: 28, gap: 12 }}>
        ...
        <View className="rounded-3xl bg-card px-4 py-4">
          <Text className="text-text-sub text-xs font-bold tracking-widest">THIS MONTH</Text>
        ...
        <View className="rounded-3xl bg-card px-4 py-4">
          <Text className="text-text-sub text-xs font-bold tracking-widest">BUDGET UTILIZATION</Text>
          {finance.insights.budgetUtilization.length === 0 ? (
            <Text className="text-text-sub mt-2 text-sm">No budgets available yet.</Text>
          ) : (
            finance.insights.budgetUtilization.slice(0, 4).map((entry) => (
              <View key={entry.budget.id} className="mt-3">
                <View className="mb-1 flex-row items-center justify-between">
                  ...
                  <Text className={`text-sm font-bold ${entry.overLimit ? 'text-red-500' : 'text-text-sub'}`}>
                    {entry.percentage}%
                  </Text>
                </View>
        ...
        <View className="rounded-3xl bg-card px-4 py-4">
          <Text className="text-text-sub text-xs font-bold tracking-widest">CATEGORY BREAKDOWN</Text>
          {finance.insights.categoryBreakdown.length === 0 ? (
            <Text className="text-text-sub mt-2 text-sm">No expense data this month yet.</Text>
          ) : (
            finance.insights.categoryBreakdown.slice(0, 5).map((entry) => (
              <View key={entry.id} className="mt-3 flex-row items-center justify-between">
                ...
                <Text className="text-text-sub text-sm">{entry.percentage}%</Text>
              </View>
            ))
          )}
        </View>
