...
export function OwedEntryCard({ debt, formatCurrency, onPressDetails, onEdit, onDelete }: OwedEntryCardProps) {
  ...
  const dueText = debt.due_date ? `Expected ${debt.due_date}` : 'No expected date';
  ...
  return (
    ...
    >
      <View style={[s.cardRail, { backgroundColor: theme.isDark ? theme.blue : '#4F7FB8' }]} />
      ...
      <View style={s.amountRow}>
        <Text style={[s.amountLabel, { color: theme.secondary }]}>Outstanding</Text>
      ...
      <View style={s.progressRow}>
        <Text style={[s.debtNumbers, { color: theme.secondary }]}>Collected {formatCurrency(collected)} / {formatCurrency(total)}</Text>
